...
/*
    private void objectToXmlTag(Tag inputTag){
        ...
        boolean isFirstTag = checkIfInitialTagIsP(currentTag);
        boolean isFamilyActive = closeFamilyTagIfActive(currentTag);
        boolean toggleFamilyActive = toggleFamilyTag(currentTag);
        ...
        switch (currentTag) {
            case "P":
                ...
                if (firstPerson) {
                    finalXmlString.append("<people>" + indent[ONE] + "<person>" + indent[TWO] + "<firstname>" + personTag.getFirstName() + "</firstname>" + indent[TWO] + "<lastname>" + personTag.getLastName() + "</lastname>");
                    firstPerson = false;
                } else {
                    finalXmlString.append(indent[ONE] + "</person>" + indent[ONE] + "<person>" + indent[TWO] + "<firstname>" + personTag.getFirstName() + "</firstname>" + indent[TWO] + "<lastname>" + personTag.getLastName() + "</lastname>");
                }
                break;
            case "T":
                PhoneTag phoneTag = (PhoneTag)inputTag;
                if (familyTagActive){
                    finalXmlString.append("\n\t\t\t<phone>\n\t\t\t\t<mobile>" + phoneTag.getMobile() + "</mobile>\n\t\t\t\t<home>" + phoneTag.getHome() + "</home>\n\t\t\t</phone>");
                } else {
                    finalXmlString.append("\n\t\t<phone>\n\t\t\t<mobile>" + phoneTag.getMobile() + "</mobile>\n\t\t\t<home>" + phoneTag.getHome() + "</home>\n\t\t</phone>");
                }
                break;
            case "A":
                AddressTag addressTag = (AddressTag)inputTag;
                if (familyTagActive) {
                    finalXmlString.append("\n\t\t\t<address>\n\t\t\t\t<street>" + addressTag.getStreet() + "</street>\n\t\t\t\t<city>" + addressTag.getCity() + "</city>\n\t\t\t\t<zipcode>" + addressTag.getZipCode() + "</zipcode>\n\t\t\t</address>");
                } else {
                    finalXmlString.append("\n\t\t<address>\n\t\t\t<street>" + addressTag.getStreet() + "</street>\n\t\t\t<city>" + addressTag.getCity() + "</city>\n\t\t\t<zipcode>" + addressTag.getZipCode() + "</zipcode>\n\t\t</address>");
                }
                break;
            case "F":
                FamilyTag familyTag = (FamilyTag)inputTag;
                finalXmlString.append("\n\t\t<family>\n\t\t\t<name>"+ familyTag.getName()+"</name>\n\t\t\t<born>" + familyTag.getBorn() + "</born>");
                break;
            default:
        }
    }
